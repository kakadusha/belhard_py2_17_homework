...
def add_user(username, login, email, password, age):
    with app.app_context():
        new_user = User(
            username=username, login=login, email=email, password=password, age=age
        )
...
def register():
    if request.method == "POST":
        ...
        #
        #     try:
        #         user_data = UserRegister.model_validate(request.form.to_dict())
        ...
        if errors:
            ...
            return render_template(
                "register.html", username=username, login=login, email=email, age=age
            )
        else:
