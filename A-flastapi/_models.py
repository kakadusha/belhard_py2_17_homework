...
class Painting(db.Model):
    ...
    # secondary=quiz_question:  Это ключевой момент для связи "многие-ко-многим".
    # SQLAlchemy не может напрямую создать связь "многие-ко-многим" между двумя таблицами.
    # Вместо этого, она использует промежуточную таблицу (junction table), которая
    # хранит пары идентификаторов из обеих таблиц. В данном случае,  quiz_question — это
    #  имя промежуточной таблицы, определенной ранее...

    # backref="question":  Это  *наиважнейшая*  часть.  backref создает обратную связь.
    # Без backref, вы могли бы получить доступ к связанным вопросам из объекта Quiz
    # через quiz.questions (если бы relationship в Quiz была определена как
    # questions = db.relationship("Question", backref="quiz")),
    # но из объекта Question вы не могли бы получить доступ к связанным тестам напрямую.

    # backref="question" создает атрибут question в модели Quiz. Это значит, что  вы сможете
    # получить доступ к *списку* вопросов, связанных с конкретным тестом,  через
    # quiz_instance.question.  Вместо того чтобы искать вопросы через таблицу quiz_question
    # вручную, вы можете получить доступ к ним через этот атрибут.
    ...
    # и список вопросов, связанных с тестом, через quiz_instance.question.  backref делает
    # код более чистым и удобным в использовании, избегая сложных запросов к базе данных
